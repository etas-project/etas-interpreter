...
// Target structure may contain shared subgraphs. Memoize only aliased edges;
...
pub(super) fn equal(left: &[CallTarget], right: &[CallTarget]) -> bool {
    type Siblings<'a> =
        std::iter::Zip<std::slice::Iter<'a, CallTarget>, std::slice::Iter<'a, CallTarget>>;
        ...
        if let Some((left, right)) = next.take() {
            ...
            match (left, right) {
                (
                    ...
                    if !already_seen(
                        a.0.as_ref().expect("live call target link"),
                        b.0.as_ref().expect("live call target link"),
                        &mut seen_nodes,
                    ) {
                        next = Some((a, b));
                    }
                ...
                (T::Composed(a), T::Composed(b)) => {
                    ...
                    if !already_seen(
                        a.0.as_ref().expect("live call target children"),
                        b.0.as_ref().expect("live call target children"),
                        &mut seen_children,
                    ) {
